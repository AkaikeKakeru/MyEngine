...
//クリアカラー
const float RenderTexture::clearColor_[4] = {
	0.25f,//R
	0.5f,//G
	0.1f,//B
	0.0f//A
...
void RenderTexture::CreateVertexBufferView() {
	HRESULT result;
	//上下左右の数値の設定
...
void RenderTexture::GenerateConstBuffer() {
	GenerateConstMaterial();
	GenerateConstTransform();
}

void RenderTexture::GenerateConstMaterial() {
	HRESULT result;
...
void RenderTexture::GenerateConstTransform() {
	HRESULT result;
...
void RenderTexture::CompileShaderFile() {
	HRESULT result;
...
void RenderTexture::AssembleVertexLayout() {
	//頂点レイアウト
	enum LayoutElement {
		Position,
		TEXCOORD,
...
void RenderTexture::AssembleGraphicsPipeline() {
	//グラフィックスパイプライン設定
...
void RenderTexture::GenerateRootSignature() {
	HRESULT result;
...
void RenderTexture::GeneratePipelineState() {
	HRESULT result;
	//パイプラインステートの生成
...
void RenderTexture::TextureCommand() {
	//SRVヒープの先頭ハンドルを取得
